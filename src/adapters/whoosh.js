...
export function adaptWhooshTeeTime(raw, defaults = {}) {
  ...
  return {
    id: String(raw.id || `whoosh-${stableCourse}-${date}-${stableTime}`),
    source: "Whoosh",
    course,
    date,
    time,
    ...
    allInPrice: Number(raw.exactPrice ?? raw.maximumPrice ?? raw.price),
...
export function adaptWhooshPayload(payload) {
  const defaults = payload.defaults || {};
  return payload.teeTimes.map((teeTime, index) => adaptWhooshTeeTime({
