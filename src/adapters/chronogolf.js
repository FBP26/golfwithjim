...
export function adaptChronogolfTeeTime(raw, defaults = {}) {
  ...
  return {
    id: String(raw.id || `chronogolf-${stableCourse}-${date}-${stableTime}`),
    source: "Chronogolf",
    course,
    date,
    time,
...
export function adaptChronogolfPayload(payload) {
  const defaults = payload.defaults || {};
  return payload.teeTimes.map(teeTime => adaptChronogolfTeeTime(teeTime, defaults));
}